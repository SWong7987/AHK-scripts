...
; ============================================================================
; ACADEMIC GAMES EQUATIONS - LOCAL HOT-SEAT PRACTICE v4.2
; AutoHotkey v2.0
;
...
BuildGui() {
    ...
    global GoalGroupingEdit, FinishGoalBtn, UndoGoalBtn, BonusBtn, NoGoalBtn
    ...
    UndoGoalBtn := MainGui.Add("Button", "x1081 y190 w95 h36", "Undo Goal")
    ...
    UndoGoalBtn.OnEvent("Click", UndoGoalCube)
    ...
    MainGui.Add("Text", "x978 y772 w302 h46", "Normal turn: click a Resource cube, then click the destination header. Click the selected cube again or press Esc to cancel. Before GOAL, click any Goal cube to return it.")
...
CubeClicked(idx, *) {
    ...
    ; Before GOAL is called, clicking a Goal cube returns that exact cube to
    ; Resources. This feels much closer to physically rearranging the Goal than
    ; forcing the player to undo every later cube first.
    if (Phase = "GoalSetting" && cube.Zone = "Goal") {
        pos := FindIndexInArray(GoalOrder, idx)
        if pos
            GoalOrder.RemoveAt(pos)
        cube.Zone := "Resources"
        GoalGroupingEdit.Value := ""
        SelectedCube := 0
        AddLog("Goal-setter returns " . CubeCode(cube) . " to Resources before saying GOAL.")
        UpdateGui()
        return
    }
...
PlaceSelected(zone, *) {
    ...
    global CurrentPlayer, LastMover, LastAction, BonusUsedThisTurn
    ...
    if (ResourceCount() = 0) {
        Phase := "Forceout"
        AddLog("Last cube moved. Forceout: all players write an Equation unless a timely IMPOSSIBLE challenge ends the shake first.")
        CurrentPlayer := 0
        BonusUsedThisTurn := false
    } else {
...
LockGoal(*) {
    ...
    goalAnalysis := ParseExpression(GoalPhysicalExpr, "Goal")
    if !goalAnalysis.OK {
        MsgBox(
            "That Goal cannot be interpreted as a legal Basic EQUATIONS expression.`n`n" . goalAnalysis.Reason . "`n`nRearrange the Goal cubes or change the physical grouping before saying GOAL.",
            "Illegal Goal"
        )
        return
    }
    ...
    if GoalWasGrouped
        ...
        AddLog("Goal left ungrouped on the mat.")
...
UndoGoalCube(*) {
    ...
    if (Phase != "GoalSetting" || GoalOrder.Length = 0)
        return
...
StartChallenge(type, *) {
    global Phase, ChallengeDDL, LastMover
...
PromptEquation(player, roleText) {
    ...
    defaultGoal := ""
    physicalAnalysis := ParseExpression(GoalPhysicalExpr, "Goal")
...
FinalizeShake(baseScores, reason) {
    ...
    global SelectedCube, BonusUsedThisTurn
...
RefreshZoneVisuals() {
    global Phase, SelectedCube, GoalOrder
...
UpdateGui() {
    ...
    if (Phase = "GoalSetting") {
        StatusText.Text := "Shake " . ShakeNumber . " | GOAL SETTER: " . PlayerName(GoalSetter) . " | Select rolled cubes, place them in GOAL, then Say GOAL."
        ...
        StatusText.Text := "Shake " . ShakeNumber . " | FORCEOUT: all players write Equations, unless a timely IMPOSSIBLE challenge ends the shake first."
    ...
    if (Phase = "GoalSetting") {
        ...
        UndoGoalBtn.Enabled := (GoalOrder.Length >= 1)
